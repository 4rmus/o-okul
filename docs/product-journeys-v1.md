...
| TENANT_ADMIN/ASSISTANT_ADMIN | Kurulum sihirbazinda kampus, seviye, sinif, ders ve donem hazirlar | PARTIAL | `apps/web/app/(app)/kurum/kurulum/setup-wizard.tsx`, `apps/api/src/me/me-setup-readiness.e2e.test.ts`, `apps/api/src/school/school.e2e.test.ts`, `apps/web/e2e-next/live-onboarding-next.spec.ts` | UAT-KURUM-01 | Faz 7 `pnpm live:onboarding:smoke` staging kaniti |
...
| TENANT_ADMIN/ASSISTANT_ADMIN | Duyuru, SMS disabled path, destek ve materyal islemlerini yurutur | PARTIAL | `apps/api/src/announcement/announcement.e2e.test.ts`, `apps/api/src/sms-batch/sms-batch.e2e.test.ts`, `apps/api/src/support-ticket/support-ticket.e2e.test.ts`, `apps/api/src/homework/homework.e2e.test.ts`, `packages/notification-adapter/src/index.test.ts`, `infra/notification-gateway/src/index.test.mjs`, `packages/db/prisma/migrations/20260808170000_add_whatsapp_consent_lifecycle/migration.sql`, `apps/api/src/whatsapp-consent/whatsapp-consent-store.test.ts`, `docs/DECISIONS.md` | UAT-KURUM-08 | SMS kapali yol kaniti ve e-posta saglayici smoke Faz 9; WhatsApp alt kapsami `CONTRACT_READY_EXTERNAL_NOT_RUN`, default-off; lifecycle DB/store local-ready, retention/purge, UI/API, send-time kontrol, outbound ledger, `ContactIdentity` numara yeniden tahsisi, keyed phone HMAC/rotasyon, staging DB sayım generator'ı, teslim uzlastirmasi ve provider smoke'u yok |
| TEACHER | Kendi sinif/ogrenci kapsaminda liste, profil ve PII-safe gunluk ozet okur | PASS | `apps/api/src/me/me-access-matrix.e2e.test.ts`, `apps/api/src/me/me-teacher-daily-brief.e2e.test.ts`, `apps/api/src/app.e2e.test.ts`, `apps/web/e2e-next/teacher-portal-contract-next.spec.ts` | UAT-TEACHER-01 | Gercek tenant rollout ve kullanici gozlemi `EXTERNAL_NOT_RUN` |
...
| STUDENT | Kendi kapsaminda PII-safe gunluk ozet, profil, odev, devamsizlik, not ve yetkili READY raporlarini okur | PASS | `apps/api/src/me/me-student-daily-brief.e2e.test.ts`, `apps/api/src/me/me-access-matrix.e2e.test.ts`, `apps/api/src/me/me-report-index.service.test.ts`, `apps/web/e2e-next/student-guardian-portal-contract-next.spec.ts` | UAT-STUDENT-01 | Gercek tenant rollout ve kullanici gozlemi `EXTERNAL_NOT_RUN` |
...
| UAT-KURUM-01 | Kurum admin sifir veriden kurulum sihirbazini tamamlar | `apps/web/app/(app)/kurum/kurulum/setup-wizard.tsx`, `apps/api/src/me/me-setup-readiness.e2e.test.ts`, `pnpm live:onboarding:smoke` | PARTIAL |
...
| UAT-KURUM-08 | Duyuru, SMS kapalı yol, destek ve materyal isleri calisir; WhatsApp default-off kalir | `pnpm notification:smoke` gerçek e-posta provider kanıtı ister; push bu release'te fail-closed kapsam dışıdır. `pnpm sms:smoke` v1 kapsam dışı SMS kapalı yolunu `provider=disabled` olarak kanıtlar; `pnpm upload-av:check`. WhatsApp append-only izin lifecycle'i DB/store düzeyinde local-ready olsa da UI/API, retention/purge, send-time kontrol, outbound ledger ve provider/staging smoke yoktur; alt kapsam `DEC-20260808-01` uyarınca `WHATSAPP_ENABLED=false` ve `CONTRACT_READY_EXTERNAL_NOT_RUN` durumundadır. | PARTIAL |
| UAT-TEACHER-01 | Ogretmen kendi kapsaminda PII-safe gunluk ozet, ogrenci ve rapor gorur | `pnpm --filter @o-okul/api exec vitest run src/me/me-teacher-daily-brief.service.test.ts src/me/me-teacher-daily-brief.e2e.test.ts src/me/me-access-matrix.e2e.test.ts src/report/report-generation.service.test.ts`, `pnpm --filter @o-okul/web exec playwright test -c playwright.next.config.ts e2e-next/teacher-portal-contract-next.spec.ts` | PASS |
...
| UAT-STUDENT-01 | Ogrenci portalda kendi PII-safe gunluk ozet, profil, odev, devamsizlik ve raporunu gorur | `pnpm --filter @o-okul/api exec vitest run src/me/me-student-daily-brief.service.test.ts src/me/me-student-daily-brief.e2e.test.ts src/me/me-access-matrix.e2e.test.ts`, `pnpm --filter @o-okul/web exec playwright test -c playwright.next.config.ts e2e-next/student-guardian-portal-contract-next.spec.ts` | PASS |
