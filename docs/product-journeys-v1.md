...
|---|---|---|---|---|---|
| SYSTEM_ADMIN | Ortam saglik, audit ve observability ekranlarini acar | PASS | `apps/web/app/(app)/sistem/**`, `apps/api/src/metrics/metrics.e2e.test.ts`, `apps/api/src/audit-log/audit-log.e2e.test.ts` | UAT-SYS-01 | Faz 9 canli dashboard kaniti |
| SYSTEM_ADMIN | Kurum listeler, arar, siralar ve sayfalar | PASS | `apps/api/src/tenant/tenant.controller.e2e.test.ts`, `apps/web/e2e-next/login-next.spec.ts` | UAT-SYS-01 | Yok |
...
| UAT-KURUM-03 | Kullanici ve davet akisi calisir | `pnpm --filter @uzman-hocam/api exec vitest run src/user-management/user-management.e2e.test.ts src/identity-invitation/identity-invitation.e2e.test.ts` | PASS |
...
- Faz 4: `UAT-KURUM-05` ve `UAT-KURUM-06` icin `scripts/check-live-exam-cycle-evidence.mjs`
