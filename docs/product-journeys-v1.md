...
| UAT-KURUM-08 | Duyuru, SMS kapalı yol, destek ve materyal isleri calisir | `pnpm notification:smoke` gerçek e-posta provider kanıtı ister; push bu release'te fail-closed kapsam dışıdır. `pnpm sms:smoke` v1 kapsam dışı SMS kapalı yolunu `provider=disabled` olarak kanıtlar; `pnpm upload-av:check` | PARTIAL |
