...
- Control-plane ekranlarının `kurum/_shared` altındaki üç salt sunum bileşenini kullanması geçici ve
  isimli allowlist borcudur. Bu bileşenler tenant auth, API veya veri adapter'ı taşımaz; `UI-01`
  sırasında ortak public UI yüzeyine alınacaktır. Gate B bu borcu kapanmış saymaz.
