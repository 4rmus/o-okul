...
| 3 | Auth + tenant context + RBAC | backend-architect | `apps/api` | Tamamlandı: auth refresh cookie rotation/reuse/logout, ALS bearer context, RBAC guard ve tenant A/B Student Supertest uçları geçti (`pnpm --filter @uzman-hocam/api test`, `pnpm run ci`) |
...
| 6 | Web shell + login + dashboard | frontend-architect | `apps/web`, `apps/api` | Tamamlandı: Vite React web shell, login formu, demo kurum paneli, API CORS ve Playwright smoke eklendi; `pnpm --filter @uzman-hocam/web build`, `pnpm test:e2e`, `pnpm run ci` geçti |
