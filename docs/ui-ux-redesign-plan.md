...
| Faz 5 gerçek staging readiness audit | `gh variable set STAGING_DEPLOY_DIR --repo 4rmus/o-okul --env staging --body /root/o-okul`, `corepack pnpm staging:github-env:gaps:summary -- --repo 4rmus/o-okul --environment staging --gap-report-file artifacts/local/staging-github-env-gap-report.json`, `ssh -o BatchMode=yes o-okul-prod 'cd /root/o-okul && set -a && . ./.env >/dev/null 2>&1 && . ./.env.release >/dev/null 2>&1 && set +a && docker compose -f docker-compose.yml -f docker-compose.release.yml -f docker-compose.traefik.yml ps --format json web api worker queue-board'` | GAP | GitHub staging var `STAGING_DEPLOY_DIR=/root/o-okul` olarak hizalandı; remote deploy gerçekliği git checkout yerine çalışan image tag'leriyle doğrulanır. Kalan GitHub environment blokajı yalnız `GHCR_READ_TOKEN` ve `STAGING_EVIDENCE_ENV_B64` secret eksikleri. |
...
| Faz 5 remote host alias guardrail'i | `ssh -o BatchMode=yes -o ConnectTimeout=5 o-okul-prod 'printf ok'`, `corepack pnpm prod:plan:check`, `corepack pnpm ops:check` | PASS/GAP ayrımı | Güncel release host alias'ı `o-okul-prod`. Remote final/gap script default'ları `/root/o-okul` remote root'una çekildi; kalan kırmızı durum host çözümleme değil, eksik final artifact. |
...
| Faz 5 remote release bundle gap audit | `corepack pnpm staging:remote-release-gaps:summary -- --host o-okul-prod --remote-root /root/o-okul --snapshot-dir artifacts/local/remote-staging-snapshot --gap-report-file artifacts/local/remote-staging-gap-report.json` | GAP, `NOT_RELEASE_EVIDENCE` | Remote API `/health` OK; deploy parity git checkout yerine çalışan image tag'leriyle doğrulanır. Snapshot'ta beklenmeyen dosya yok; 9 zorunlu dosya, 10 açık kapanış kalemi ve `release-summary-*.json` eksik. Eksikler deployment-rollback, identity-migration, financial-retention, observability-uat, external-monitoring, admin-mfa, security-audit, ui-ux-redesign, UAT ve release-summary kanıtlarında kalır. |
...
- Web ve API response içinde `X-Powered-By` görünmemelidir.
...
- Path/slug patch'i main'e push edilir; deploy sonrası çalışan image tag parity'si doğrulanır ve deploy dispatch öncesi aynı preflight yine çalıştırılır.
