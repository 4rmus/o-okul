...
| Faz 5 UI/UX canlı yüzey kapısı | `corepack pnpm ui-ux-redesign:live-surface:check -- --base-url https://o-okul.com`, `curl -skI https://o-okul.com/login`, `curl -sk https://o-okul.com/health/ready` | PASS | Canlı `o-okul.com` üzerinde `/login`, `/health`, `/health/ready`, HTTPS güvenlik başlıkları, web CSP ve `X-Powered-By` kapanışı doğrulandı. Netgsm, S3, Sentry bu kapının girdisi değildir; onlar full production/go-live kapısında kalır. |
