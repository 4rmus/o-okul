...
const rolePortalViewportContractName =
  "rol portal aksiyon şeritleri 320/375/414/768/1024/1440 görünümde taşmadan kalır";

test.describe("Faz 9 UI görsel smoke", () => {
  test("landing 320/375/414/768/1024/1280/1440 görünümde gerçek ürün akışını taşmadan gösterir", async ({ page }) => {
  ...
  test("genel giriş kartı 414px deterministik görsel sözleşmesini korur", async ({ page }) => {
    await page.setViewportSize({ height: 896, width: 414 });
    await page.goto("/login");
    const loginPanel = page.getByRole("region", { name: "Giriş" });
    await expect(loginPanel).toBeVisible();
    await hideNextDevIndicator(page);
    await expect(loginPanel).toHaveScreenshot("login-panel-414.png", {
      animations: "disabled",
  ...
  test("kurum dashboard 320/375/414/768/1024/1440 görünümde özet, karar ve rapor sözleşmesini korur", async ({ page }) => {
    ...
    for (const viewport of redesignViewports) {
      ...
      const attentionRegion = page.getByRole("region", { exact: true, name: "Bugün ilgilenmeniz gerekenler" });
      await expect(attentionRegion).toContainText("Bugün ilgilenmeniz gerekenler");
      await expect(attentionRegion.locator(".next-attention-item.uh-action-card")).toHaveCount(3);
      await expect(attentionRegion.getByRole("link", { name: /Öğrenci destek talepleri 1/ })).toHaveAttribute("href", "/kurum/destek");
      await expect(attentionRegion.getByRole("link", { name: /Devamsızlık takibi 1/ })).toHaveAttribute("href", "/kurum/devamsizlik");
      await expect(attentionRegion.getByRole("link", { name: /Sonuç kontrolü 1/ })).toHaveAttribute("href", "/kurum/optik");
      ...
      await expect(dashboardSummary).toContainText("Aktif öğrenci");
      await expect(dashboardSummary).toContainText("Son sınav katılımı");
      await expect(dashboardSummary).toContainText("Son sınav başarı yüzdesi");
      await expect(dashboardSummary).toContainText("Destek bekleyen iş");
      await expect(dashboardSummary).not.toContainText("Sistem sağlığı");
      const announcementsRegion = page.getByRole("region", { exact: true, name: "Kurum duyuruları" });
      await expect(announcementsRegion).toContainText("Haftalik deneme programi");
      await expect(announcementsRegion.getByRole("link", { name: "Tüm duyuruları aç" })).toHaveAttribute("href", "/kurum/duyurular");
      const latestExamRegion = page.getByRole("region", { exact: true, name: "Son sınav ve rapor durumu" });
      const dashboardSurfaceOrder = attentionRegion.or(dashboardSummary).or(announcementsRegion).or(latestExamRegion);
  ...
  test("komut paleti link semantiğiyle modül geçişini korur", async ({ page }) => {
    ...
    const paymentsResult = commandDialog.getByRole("link", { name: /Ödeme planları/ });
    await expect(paymentsResult).toBeVisible();
    await expect(commandDialog.getByRole("button", { name: /Ödeme planları/ })).toHaveCount(0);
  ...
  test("öğrenci listesi 320/375/414/768/1024/1440 görünümde URL state ile taşmadan kalır", async ({ page }) => {
    test.setTimeout(90_000);
    const consoleErrors = collectConsoleErrors(page);
  ...
  test("veli detay desktop ve mobil izin ilişkilerini güvenli gösterir", async ({ page }) => {
    ...
    const guardianSummary = guardianDetail.getByRole("region", { exact: true, name: "Veli detay özeti" });
    ...
    await expect(guardianSummary).toContainText("Kişisel bilgiler maskeli");
    await expect(guardianSummary.getByLabel("Veli detay özeti önerilen işlemler")).toBeVisible();
  ...
  test("öğretmen detay desktop ve mobil görev ilişkilerini güvenli gösterir", async ({ page }) => {
    ...
    await expect(teacherSummary.getByLabel("Öğretmen detay operasyon özeti önerilen işlemler")).toBeVisible();
  ...
  test("sınıf detay desktop ve mobil rapor bağlamını güvenli gösterir", async ({ page }) => {
    ...
    await expect(classSummary.getByLabel("Sınıf detay operasyon özeti önerilen işlemler")).toBeVisible();
  ...
  test("öğrenci detay desktop ve mobil kanıtları ilişki haritası fallback'iyle temizdir", async ({ page }) => {
    ...
    const studentSummary = studentDashboard.getByRole("region", { exact: true, name: "Öğrenci detay özeti" });
    ...
    await expect(studentSummary).toContainText("Kişisel bilgiler maskeli");
    await expect(studentSummary.getByLabel("Öğrenci detay özeti önerilen işlemler")).toBeVisible();
    ...
    const studentExamSummary = studentExamDetails.getByRole("region", { exact: true, name: "Öğrenci sınav özeti" });
    ...
    await expect(studentExamSummary.getByLabel("Öğrenci sınav özeti önerilen işlemler")).toBeVisible();
  ...
  for (const viewport of redesignViewports) {
    test(`${rolePortalViewportContractName} · ${viewport.width}px`, async ({ page }) => {
      const consoleErrors = collectConsoleErrors(page);

      for (const portalCase of rolePortalActionStripCases) {
        ...
        if (portalCase.key === "student" && viewport.width === 414) {
          await hideNextDevIndicator(page);
          await expect(actionStrip).toHaveScreenshot("student-portal-actions-414.png", {
            animations: "disabled",
            maxDiffPixelRatio: 0.005,
          });
          await expectRouteFamilyGolden(
  ...
  test("rapor route family golden sözleşmesini korur", async ({ page }) => {
    ...
    await expect(statusSurface).toContainText("Rapor hazır");
    const analyticsPanel = page.getByRole("tabpanel", { name: "Genel Bakış" });
    await expect(analyticsPanel.locator(".uh-chart-loading")).toHaveCount(0);
    await expect(analyticsPanel.locator("canvas")).toHaveCount(2);
    await page.waitForTimeout(200);
  ...
  test("rapor çalışma alanı 320/375/414/768/1024/1440 görünümde bağlam ve karne taşmadan kalır", async ({ page }) => {
    ...
    for (const viewport of redesignViewports) {
      ...
      await expect(karneContext.getByRole("group", { name: "Karne rapor ölçüleri" })).toHaveClass(/uh-info-grid/);
      ...
      if (viewport.width === 1024) {
        ...
        const printKarneBox = await karneSheet.boundingBox();
  ...
  test("optik workflow 320/375/414/768/1024/1440 görünümde tab semantiği ve yoğun form düzenini korur", async ({ page }) => {
    ...
    for (const viewport of redesignViewports) {
      ...
      await expect(reportHandoff.getByRole("link", { name: "Rapor çalışma alanına geç" })).toHaveCount(0);
      await expect(reportHandoff.getByRole("button", { name: "Analiz tamamlanınca açılır" })).toBeDisabled();
...
function createInstitutionDashboardSummary() {
  return {
    ...
    latestExam: {
      ...
      report: {
        ...
        questionCount: 30,
        classes: [
          { classId: "class-8a", className: "8-A", resultCount: 1, successRate: 81.7, net: 24.5, questionCount: 30 },
          { classId: "class-8b", className: "8-B", resultCount: 1, successRate: 63.3, net: 19, questionCount: 30 },
        ],
      },
    },
  };
...
async function expectRolePortalActionStrip(
  ...
  await expect(actionStrip.locator(".next-portal-action-strip__item")).toHaveCount(Math.min(3, expectedCount));
  await expect(actionStrip.locator(".next-portal-action-strip__secondary-item")).toHaveCount(Math.max(0, expectedCount - 3));
