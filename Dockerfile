...
RUN pnpm turbo run build --filter=@uzman-hocam/web...

FROM deps AS build-api
RUN pnpm turbo run build --filter=@uzman-hocam/api...

FROM deps AS build-worker
RUN pnpm turbo run build --filter=@uzman-hocam/worker...

FROM deps AS build-queue-board
RUN pnpm turbo run build --filter=@uzman-hocam/queue-board...
