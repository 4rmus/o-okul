...
export function assertProductEvent(input: unknown, context: ProductEventValidationContext = {}): ProductEvent {
  ...
  for (const field of ["errorCode", "correlationId"] as const) {
    const value = input[field];
    if (value !== undefined && (typeof value !== "string" || !safeToken.test(value) || unsafeValue(value))) {
      fail("PRODUCT_EVENT_" + field.toUpperCase() + "_INVALID");
    }
  }
