...
for (const invalidConfigSource of [
  playwrightConfigSource.replace("pnpm next:build", "if [ ! -f .next/BUILD_ID ]; then pnpm next:build; fi"),
...
if (
  ...
  || environment.nextBuild !== "DEDICATED"
...
for (const task of tasks) {
  ...
  for (const sample of task.samples) {
    ...
    for (const metric of ["durationMs", "fcpMs", "lcpMs", "ttfbMs"]) {
      if (!(sample[metric] > 0)) failures.push(task.taskId + ": " + metric + " pozitif olmalı");
    }
  }
  for (const metric of metrics) {
...
function validateMeasurementRuntimeConfig(source) {
  ...
  if (!source.includes("&& pnpm next:build &&")) output.push("measurement dedicated Next build zorlaması eksik");
