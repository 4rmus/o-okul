...
import { createTenantPseudonym } from "../packages/shared-types/dist/product-analytics-server.js";
...
const trustedTenantPseudonym = createTenantPseudonym("tenant-a", "k".repeat(32));
assertProductEvent(
...
try {
  createTenantPseudonym("tenant-a", "too-short");
} catch {
  weakKeyRejected = true;
}
if (!weakKeyRejected) throw new Error("TENANT_PSEUDONYM_WEAK_KEY_ACCEPTED");

console.log("Product analytics schema kontrolü geçti: 2 pozitif, " + (rejected.length + 1) + " negatif fixture.");
