...
const expectations = {
  ...
  "docker-compose.traefik.yml": [
    ...
    "web:",
    "traefik.http.routers.web.rule=Host(`${DOMAIN}`) || Host(`*.${DOMAIN}`)",
    "traefik.http.routers.web.entrypoints=websecure",
    "traefik.http.routers.web.priority=10",
    "traefik.http.routers.web.service=web",
    "traefik.http.routers.web.tls.certresolver=letsencrypt",
    "traefik.http.routers.web.tls.domains[0].sans=*.${DOMAIN}",
    ...
    "PathPrefix(`/health`)",
    "Host(`*.${DOMAIN}`)",
    "traefik.http.routers.api.rule=(Host(`${DOMAIN}`) || Host(`*.${DOMAIN}`)) && (PathPrefix(`/api`) || PathPrefix(`/health`))",
