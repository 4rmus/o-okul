...
const secretName = "GHCR_READ_TOKEN";
const args = process.argv.slice(2);
const tokenFileArg = readArgValue("--token-file");
...
if (!tokenFileArg) {
  failures.push("--token-file için özel GHCR read token dosyası gerekli.");
}
if (!repo) {
...
const tokenFile = resolve(tokenFileArg);
validateSecretFile(tokenFile, failures);
...
validateTokenValue(token, failures);
...
if (dryRun) {
  console.log(`Staging GHCR read token secret dry-run geçti: ${repo}/${environmentName}/${secretName}`);
  process.exit(0);
}
...
console.log(`GitHub staging GHCR read token secret güncellendi: ${repo}/${environmentName}/${secretName}`);
...
function validateTokenValue(value, output) {
  if (!value) {
    output.push("GHCR read token boş olmamalı.");
    return;
  }
  if (value.length < 20) {
    output.push("GHCR read token en az 20 karakter olmalı.");
  }
  if (/\s/.test(value)) {
    output.push("GHCR read token whitespace içermemeli.");
  }
  if (/__SET_|replace-me|example|placeholder|redacted/i.test(value)) {
    output.push("GHCR read token gerçek değer olmalı.");
  }
}
...
Options:
  ...
  --dry-run       Dosya güvenliğini doğrular, secret yazmaz.

Secret değeri gh secret set komutuna stdin üzerinden verilir; shell argümanına yazılmaz.
`);
}

function fail(messages) {
  console.error("Staging GHCR read token secret set işlemi başarısız:");
