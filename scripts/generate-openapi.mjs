...
const sharedTypeDriftContracts = [
  ...
  { interfaceName: "AnnouncementRecipientPreviewRequest", method: "post", path: "/api/v1/announcements/recipients/preview", schemaPath: ["requestBody"] },
  { interfaceName: "AnnouncementRecipientPreviewResult", method: "post", path: "/api/v1/announcements/recipients/preview", schemaPath: ["responseData"] },
...
const requiredOperationContracts = [
  ...
  {
    method: "post",
    path: "/api/v1/learning-outcomes/imports",
    requestBody: true,
    responseEnvelope: true,
    idempotencyHeader: true,
    requestRequired: ["fileBase64"],
    responseDataRequired: ["importedRows", "createdOutcomes", "updatedOutcomes", "outcomes"],
    responseDataForbiddenDeep: ["fileBase64"],
    fieldChecks: [
      { path: ["requestBody", "fileBase64"], minLength: 1 },
      { path: ["responseData", "importedRows"], minimum: 0 },
      { path: ["responseData", "createdOutcomes"], minimum: 0 },
      { path: ["responseData", "updatedOutcomes"], minimum: 0 },
    ],
  },
  {
    method: "get",
  ...
  {
    method: "get",
    path: "/api/v1/me/teacher/daily-brief",
    responseEnvelope: true,
    ...
    fieldChecks: [
      ...
      { path: ["responseData", "actions"], maxItems: 3 },
      { path: ["responseData", "actions", "items", "id"], enum: ["attendance", "homework", "report", "support"] },
      { path: ["responseData", "nextLesson", "startsAt"], format: "date-time" },
      { path: ["responseData", "nextLesson", "endsAt"], format: "date-time" },
      { path: ["responseData", "latestReadyReport", "latestGeneratedAt"], format: "date-time" },
    ],
  },
  {
    method: "get",
  ...
  },
  {
    ...
    requestBody: true,
    responseEnvelope: true,
    requestRequired: ["loginName", "password"],
  ...
  },
  {
    ...
    requestBody: true,
    responseEnvelope: true,
    requestRequired: ["studentNo", "code", "password"],
  ...
  },
  {
    ...
    responseDataForbiddenDeep: ["tenantId", "startsAt", "expiresAt", "reference"],
  },
  {
    method: "get",
  ...
  },
  {
    ...
    responseEnvelope: true,
    idempotencyHeader: true,
    requestRequired: ["body", "channel", "recipientPreviewToken", "title"],
    responseDataRequired: ["id", "tenantId", "title", "body", "audience", "publishedAt"],
    fieldChecks: [
      { path: ["requestBody", "audience"], enum: announcementAudiences },
      { path: ["requestBody", "channel"], enum: ["IN_APP"] },
      { path: ["responseData", "audience"], enum: announcementAudiences },
    ],
  },
  {
    method: "post",
    path: "/api/v1/announcements/recipients/preview",
    requestBody: true,
    responseEnvelope: true,
    requestRequired: ["channel"],
    responseDataRequired: ["audience", "channel", "counts", "expiresAt", "previewToken", "recipientCount", "scope"],
    fieldChecks: [
      { path: ["requestBody", "audience"], enum: announcementAudiences },
      { path: ["requestBody", "channel"], enum: ["IN_APP"] },
      { path: ["responseData", "audience"], enum: announcementAudiences },
      { path: ["responseData", "channel"], enum: ["IN_APP"] },
      { path: ["responseData", "expiresAt"], format: "date-time" },
    ],
  },
  {
